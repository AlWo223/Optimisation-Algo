...
def main():
    n = int(input().strip()) # number of scenarios

    ### Multiple scenarios
    output = list()
    for ii in range(0, n):
        h, w = map(int, input().strip().split(" ")) # height and width of grid

        ### Store graph
        graph = [[] for ii in range(0, h)]
        for xx in range(0, h):
        ...
        for xx in range(0, h):
            for yy in range(0, w):
                whiteCell = ((xx+yy)%2 == 0)
                node = xx*w+yy
                #print("Node: ", node, "White: ", whiteCell)
                if(graph[xx][yy]=='*'):
                    if(whiteCell):
                        mf.add_edge(s, node, 1)
                        neighbours = [(xx-1, yy), (xx, yy+1), (xx+1, yy), (xx, yy-1)]
                        for (uu, vv) in neighbours:
                            if((uu>=0) and (uu<h) and (vv>=0) and (vv<w)):
                                index = uu*w+vv
                                mf.add_edge(node, index, 1)
                    else:
                        mf.add_edge(node, t, 1)
                #print(mf)

        output.append(points-mf.dinic(s, t))
