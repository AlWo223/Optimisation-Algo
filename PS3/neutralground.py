...
def main():
    ...
    #counter = -1
    for xx in range(0, h):
        for yy, square in enumerate(input().strip()):
            inNode = xx*w + yy
            outNode = inNode + w*h
            if(square=="A"):
                mf.add_edge(2*w*h, inNode, INF, True)
                #mf.add_edge(outNode, 2*w*h, INF, True)
                mf.add_edge(inNode, outNode, INF, True)
            elif(square=="B"):
                mf.add_edge(outNode, 2*w*h+1, INF, True)
                #mf.add_edge(2*w*h+1, inNode, INF, True)
                mf.add_edge(inNode, outNode, INF, True)
            else:
                cap = int(square)
                mf.add_edge(inNode, outNode, cap, True)
            for (uu, vv) in [(xx-1, yy), (xx, yy+1), (xx+1, yy), (xx, yy-1)]:
                if((uu>=0) and (uu<h) and (vv>=0) and (vv<w)):
                    index = uu*w+vv
                    mf.add_edge(outNode, index, INF, True)
            #print(mf)

    print(mf.dinic(w*h*2, 2*w*h+1))
